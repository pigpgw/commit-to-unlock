...
1. Android UI를 raw `LinearLayout`에서 작은 view factory/helpers로 정리한다.
2. `DogfoodEventStore`에 event type 상수 또는 sealed class를 도입할지 검토한다.
3. `packages/shared`의 `MobileCreditState`를 Android/iOS 문서와 자동 비교하는 작은 contract test를 추가한다.
4. iOS skeleton은 Xcode project 생성 전까지 문서 수준으로만 유지한다.
5. Sprint 4 착수 전 `apps/api`에 persistence/dedupe 설계부터 추가하고, webhook route는 enrichment와 ledger가 함께 준비될 때만 복구한다.
